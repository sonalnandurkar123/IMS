...
if(isset($_GET['id']))
	{		
			$id=$_GET['id'];	
			$stmt_stu=$conn->prepare("DELETE FROM student_details WHERE student_id=$id");
			$executed=$stmt_stu->execute();
            $stmt_edu=$conn->prepare("DELETE FROM edu_performance WHERE stud_id=$id");
			$executed=$stmt_edu->execute();
            $stmt_family=$conn->prepare("DELETE FROM family_info WHERE stud_id=$id"); 
            $executed=$stmt_family->execute();			
            $stmt_trans=$conn->prepare("DELETE FROM daily_transaction WHERE student_id=$id");	
            $executed=$stmt_trans->execute();
            $stmt_pay=$conn->prepare("DELETE FROM student_payment_details WHERE student_id=$id");	
            $executed=$stmt_pay->execute();	
            $stmt_attendance=$conn->prepare("DELETE FROM attendance_details WHERE student_id=$id");	
            $executed=$stmt_attendance->execute();	
            $stmt_exam=$conn->prepare("DELETE FROM exam_marks WHERE student_id=$id");	
            $executed=$stmt_exam->execute();				
				if($executed)
                      {
        ...
        <div class="right_col" role="main">
          <div class="">
		    ...
		    <div class="col-md-12 col-sm-12 ">
				<div class="x_panel">
					<div class="x_title">
						<a href="add_student" class="btn btn-primary submit" >Add Student</a>
					...
					<div class="x_content">
									    ...
									    <tbody>
											...
											<tr>
												...
												<td>
													...
													<a href="candidate_detail?id=<?php echo $row_student[$c]['student_id']?>" data-toggle="tooltip" data-placement="bottom" title="View" target="_blank"><i class="fa fa-eye fa-1x text-info"></i></a>&nbsp;
								
													
													<a href="" data-toggle="modal" data-target="#myModal<?php echo $row_student[$c]['student_id'];?>"  data-toggle="tooltip" data-placement="bottom" title="Payment"><i class="fa fa-credit-card fa-1x text-primary"></i></a>&nbsp;			
		
													<a href="student_list?id=<?php echo $row_student[$c]['student_id'];?>" data-toggle="tooltip" data-placement="bottom" title="Delete" onclick="return confirm('Are you sure you want to delete this?');"><i class="fa fa-trash-o fa-1x text-danger"></i></a>
												</td>
											</tr>
									<div  id="myModal<?php echo $row_student[$c]['student_id'];?>" class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
									<div  class="modal-dialog">
								<form method="post" action="print_receipt_update" novalidate>
						...
						<div class="field item form-group">
							...
							<div class="col-sm-6">
								<input type="hidden" value="<?php echo $row_student[$c]['student_id'];?>" name="student_id">
								<input type="text" class="form-control"  placeholder="Enter Student Name" value="<?php echo $row_student[$c]['student_name'];?>" name="student_name" readonly>
							</div>
						</div>
						<div class="field item form-group">
							...
							?>
								...
								<input type="hidden" class="form-control"  placeholder="Enter Course Fees" value="<?php echo $row_student[$c]['course_details_id'];?>" name="course_details_id" readonly>
							</div>
						</div>				
                        <div class="field item form-group">
							<label for="focusedinput" class="col-form-label col-md-3 col-sm-3 label-align">Course Fees</label>
							<div class="col-sm-6">
								<input type="text" class="form-control"  placeholder="Enter Course Fees" value="<?php echo $row_student[$c]['course_fees'];?>" name="course_fees" readonly>
							</div>
						</div>
						<div class="field item form-group">
							<label for="focusedinput" class="col-form-label col-md-3 col-sm-3 label-align">Paid Fees</label>
							<div class="col-sm-6">
								<input type="text" class="form-control"  placeholder="Enter Paid Fees" name="paid_amount" value="<?php echo $row_student[$c]['paid_amount'];?>" readonly>
							</div>
						</div>
						<div class="field item form-group">
							<label for="focusedinput" class="col-form-label col-md-3 col-sm-3 label-align">Balance Fees</label>
							<div class="col-sm-6">
								<input type="text" class="form-control"  placeholder="Enter Balance Fees" value="<?php echo $row_student[$c]['balance_amount'];?>" name="balance_amount" readonly>
							</div>
						</div>	
						<div class="field item form-group">
